...
export default function EditorPadel() {
  ...
  useEffect(() => {
    if (!loading) {
      const padelData = eventos.find((e) => e.evento === "padel");
      if (padelData) {
        const selected = selectedDays[0];
        setPrecioDia(padelData.preciosPorDia[selected] || "");
        setPrecioConBuffet(padelData.precioconbuffet || "");
      }
    }
